...
public class Task4 {
	public double getSimpleClassifierKroswalidation(int beginA, int endA, int beginB, int endB, String choose,
			double[][] matrixA, double[][] matrixB, double[][] matrixToTrainingA, double[][] matrixToTrainingB,
			int knnValue) {
		...
		if (choose.equals("KNN")) {
			...
			for (int i = 0; i < matrixA[0].length; i++) {
				if (i < beginA || (i > endA && endA < matrixA[0].length)) {
					suma++;
					if (task.KNN(matrixToTrainingA, matrixToTrainingB, knnValue, changeColumnToRow(matrixA, i))
							.equals("a")) {
						wynik++;
					}
				}
			...
			for (int i = 0; i < matrixB[0].length; i++) {
				if (i < beginB || (i > endB && endB < matrixB[0].length)) {
					suma++;
					if (task.KNN(matrixToTrainingA, matrixToTrainingB, knnValue, changeColumnToRow(matrixB, i))
							.equals("b")) {
						wynik++;
					}
				}
	...
	public double getSimpleClassifierBootstrap(String choose, double[][] matrixA, double[][] matrixB,
			double[][] matrixToTrainingA, double[][] matrixToTrainingB, int knnValue, int[] trainIndex,
			int[] trainIndexB) {
		int countA = 0;
		int countB = 0;
		Task3 task = new Task3();
		...
		double skutecznoscA = (double) countA / sum * 100;
		double skutecznoscB = (double) countB / sumB * 100;
		return (skutecznoscA + skutecznoscB) / 2;
	}

	public double getSimpleClassifier(int beginA, int endA, int beginB, int endB, String choose, double[][] matrixA,
			double[][] matrixB, double[][] matrixToTrainingA, double[][] matrixToTrainingB, int knnValue) {
		...
		double skutecznoscA = (double) countA / difference * 100;
		double skutecznoscB = (double) countB / differenceB * 100;
		return (skutecznoscA + skutecznoscB) / 2;
	}
